...
Wpx= 80^2; Wpy =100^2;
Wv=50^2;
Ig=map(tx,ty,ipx,ipy,xp,yp,(iv/vp)); %Initial Intercept point based on the sensor data
...
Igactual=map(tx,ty,xtru,ytru,xp,yp,vtru/vp);
Igxe=Igactual(1);
Igye=Igactual(2);
 ...
 for t=1:T
   ...
   covy(end+1)=s(end).P(3,3);
    pursuer=mapupdated(Ig(1),Ig(2),s(end-1).x(1),s(end-1).x(3),xp(end),yp(end),vp,delT); % end+1 because initially we stored initial value of pursuer, and we used map function once
   so the next instant of pursuer is updated accordingly
   xp(end+1)=pursuer(1);
   yp(end+1)=pursuer(2);
   updated.
   Ig=map(tx,ty,s(end).x(1),s(end).x(3),xp(end),yp(end),(s(end).x(2))/vp); % end-1 because our current values,which are needed for calculating interception point are in s(end-1) structure
   ...
   Igevader=map(tx,ty,tru(end-2),tru(end),xp(end),yp(end),(tru(end-1))/vp); % end-1 because our current values,which are needed for calculating interception point are in s(end-1) structure
