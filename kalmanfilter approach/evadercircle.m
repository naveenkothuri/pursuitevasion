...
function I= evadercircle(tx,ty,xe,ye,xp,yp,k)
figure
xei=xe;
%{
xe=xe-0.5;
...
d=min(d1,d2);
X3 = [double(solx(1,1)),double(soly(1,1));xe,ye];
d3 = pdist(X3,'euclidean');
X4 = [double(solx(2,1)),double(soly(2,1));xe,ye];
...
if((xp~=e))
    xp=xp+sign(e-xp)*(1/k)*d5*cos(atan(a2)); %corresponding to distance moved by evader
yp=(a2)*(xp)+b2;
end
end
I=[solxm;solym];
