...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("lego_move_optitrack2", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(lego_move_optitrack2_M) == (NULL)) &&
...
int main(int argc, char **argv)
{
  ...
  if (rtmGetErrorStatus(lego_move_optitrack2_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(lego_move_optitrack2_M));
  }
