 ...
 for t=1:T
  ...
  % the outupt of kalman filter is x(2) given two measurements.
   ...
   covy(end+1)=s(end).P(3,3);
    pursuer=mapupdatedkal(Ig(1),Ig(2),s(end-1).x(1),s(end-1).x(3),xp(end),yp(end),vp,delT); % end+1 because initially we stored initial value of pursuer, and we used map function once
   %so the next instant of pursuer is updated accordingly
   xp(end+1)=pursuer(1);
   yp(end+1)=pursuer(2);
   %updated.
   Ig=mapkal(tx,ty,s(end).x(1),s(end).x(3),xp(end),yp(end),(s(end).x(2))/vp); % end-1 because our current values,which are needed for calculating interception point are in s(end-1) structure
