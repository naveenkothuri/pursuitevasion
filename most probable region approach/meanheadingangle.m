...
%map1(tx,ty,x0(end),y0(end),xp(end),yp(end),sx,sk,v0(end)/vp)
%figure
for t=1:T
Ievader=mapkal(tx,ty,xetru(end),yetru(end),xp(end),yp(end),ketru);
Igevaderx(end+1)=Ievader(1);Igevadery(end+1)=Ievader(2);
alphapursuer= abscissameanheadingangle(x0(end),y0(end),v0(end),sx,sv,xp(end),yp(end),tx,ty,vp);
if(isnan(alphapursuer))
    alphapursuer
end
if hh~=1
...
xetru(end+1)=double(E(1));yetru(end+1)=double(E(2));
 P=[xp(end)+cos(alphapursuer*pi/180)*vp*delT;yp(end)+sin(alphapursuer*pi/180)*vp*delT];
