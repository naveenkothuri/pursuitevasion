function I= abscissameanheadingangle(x0,y0,v0,sx,sv,xp,yp,xt,yt,vp)
...
ang=@(x,y,k)wrapTo360(atan2((ycor1(x,y,k)-yp),(xcor1(x,y,k)-xp))*180/pi);
fun1=@(x,y,k)(wrapTo360(atan2((ycor1(x,y,k)-yp),(xcor1(x,y,k)-xp))*180/pi).*exp((-1./(2.*sx.^2)).*((x-x0).^2+(y-y0).^2)).*exp((-1./(2.*sk.^2)).*((k-k0).^2))./(((2.*pi).^1.5).*(sx.^2).*sk));
...
plot(xcor1(x,y,2),ycor1(x,y,2),'*')
     %}
m=2;
I(1)=integral3(fun1,-m*sx+x0,m*sx+x0,-m*sx+y0,m*sx+y0,-m*sk+k0,m*sk+k0,'AbsTol', 0,'RelTol',1e-2);
if(isnan(I))
    I=integral3(fun1,-m*sx+x0,m*sx+x0,-m*sx+y0,m*sx+y0,-m*sk+k0,m*sk+k0,'AbsTol', 0,'RelTol',1e-0);
end
