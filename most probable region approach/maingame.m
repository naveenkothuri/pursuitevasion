...
soly=@(x,y,k)(yt+((yt.*(1-k.^2)-y+yp(end).*k.^2)./(xt.*(1-k.^2)-x+xp(end).*k.^2)).*(xcor1(x,y,k)-xt));    
...
I(1,end+1)=s.I(maxindex);
I(2,end)=s.I(maxindex);
coefficients = polyfit([I(1,end), xp(end)], [I(2,end), yp(end)], 1);
ap = coefficients (1);
bp = coefficients (2);
xp(end+1)=xp(end)+sign(I(1,end)-xp(end))*delT*vp*cos(atan(theta));
